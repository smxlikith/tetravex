...
import { formatTime, formatRelativeTime, getTopScores, saveScore } from "../../lib/utils";

export default function WinScreen({ time, onClose, onSaveScore }) {
	const topScores = getTopScores();
  const [username, setUsername] = useState("");

  const handleSave = () => {
    if (!username.trim()) return;
    saveScore(username.trim(), time);
    setUsername(""); 
	onClose();
  };
