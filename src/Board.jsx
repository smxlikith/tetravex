...
function Board() {
	...
	function checkGameWin(){
		let currBlocks = gameState.flat().reduce(
			(prev, curr)=>{
				if(curr!=null){
					prev += 1;
				}
	...
	return (
		<div className="board-container">
			...
			{!running && !showMenu && !hasWon && !showSolution && !showHelp ? 
				<Overlay>
					<div className="flex items-center justify-center">
						<button className="mx-2" onClick={toggleTimer}><Play size={26}/></button>	
						<button className="mx-2" onClick={()=>{setShowMenu((prev)=>!prev)}}><Menu size={26}/></button>	
					</div>
				</Overlay> 
				: null
			}		
			{
				showMenu?
					<Overlay>
						<GameMenu showSolution={handleShowSolution} toggleMenu={toggleGameMenu} restGame={resetBoard} changeSize={setSize} newGame={startNewGame}/>
					</Overlay>
				:null
			}
			<Timer running={running} resetSignal={resetSignal} seconds={seconds} setSeconds={setSeconds}/>
			<div className="flex justify-between">
				<button className="p-2" onClick={toggleTimer}><Pause/></button> 
				<button className="p-2 mx-2" onClick={toggleGameMenu}><Menu/></button> 
				<button className="p-2 text-lg font-bold" onClick={toggleGameHelp}><HelpCircle/></button> 
			</div>
			<DndContext
				sensors={sensors}
			...
			>
				<div className="flex flex-col sm:flex-row justify-center content-center">
					<Grid state={showSolution ? solution : gameState} grid_name={"game"} disabled={disableDrag}/>
					<div id="seperator"></div>
					<Grid state={showSolution ? Empty : shuffled} grid_name={"shuffled"} disabled={disableDrag}/>
				</div>
				<DragOverlay>
					{activeElement && (
